...
import { CreatePaymentIntentApiResponseDto, PaymentApiResponseDto } from './dto/payment_response.dto';
...
export class PaymentsController {
  ...
  @ApiCreatedResponse({ description: 'Payment intent created successfully',
  ...
  @Post('confirm')
  @ApiOperation({ summary: 'Confirm payment for a given order',
    description: 'Confirm payment intent for an order',
  })
  @ApiResponse({ description: 'Payment confirmed successfully', status: 200,
  ...
  @ApiOkResponse({ description: 'Payment recieved successfully',
    type: PaymentApiResponseDto,
  })
  ...
  @Get(':id')
  @ApiParam({ name: 'id', description: 'Payment Id', example: '154sd4848ds5d-4654-4sdd8s7d-sd4656'})
  @ApiOperation({ summary: 'Get  payments by id',
    description: 'Get specific payment details by its id'})
  @ApiOkResponse({ description: 'Payment retreived successfully',
    type: PaymentApiResponseDto,
  })
  @ApiNotFoundResponse({ description: 'Payment not found' })
  async getPaymentById(@Param('id') paymentId: string, @GetUser('id') userId: string) {
