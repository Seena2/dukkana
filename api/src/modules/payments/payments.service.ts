...
export class PaymentsService {
  ...
  constructor(private prisma: PrismaService) {
    ...
    this.stripe = new Stripe(process.env.STRIPE_SECRET_KEY!, { apiVersion: '2026-03-25.dahlia'});
  }

  //Helper function to  transform and format the response object to match the expected structure
  private mapToPaymentResponse(payment: { id: string; orderId: string; userId: string; amount: Prisma.Decimal;
    currency: string; status: PaymentStatus; paymentMethod: string | null; transactionId: string | null;
    createdAt: Date; updatedAt: Date; }): PaymentResponseDto {
    return {
  ...
  async createPaymentIntent(userId: string, createPaymentIntentDto: CreatePaymentIntentDto): Promise<{
    success: boolean; data: {clientSecret: string; paymentId: string;};  message: string; }> {
    ...
    const isPaymentCompleted = await this.prisma.payment.findFirst({where: { id: orderId } });
    if (isPaymentCompleted && isPaymentCompleted.status === PaymentStatus.COMPLETED) {
    ...
    const payment = await this.prisma.payment.create({
      data: { orderId, userId, amount, currency,
    ...
    return { success: true,
      data: { clientSecret: paymentIntent.client_secret!, paymentId: payment.id },
  ...
  async confirmPayment(userId: string, confirmPaymentDto: ConfirmPaymentDto): Promise<{
    ...
    const payment = await this.prisma.payment.findFirst({ where: { orderId, userId, transactionId: paymentIntentId }});
    ...
    const [updatedPayment] = await this.prisma.$transaction([
      this.prisma.payment.update({where: { id: payment.id}, data: { status: PaymentStatus.COMPLETED }}),
      this.prisma.order.update({where: { id: orderId}, data: { status: 'PROCESSING' }}),
    ]);
    const order= await this.prisma.order.findFirst({ where: {id: orderId}});
    // mark the cart associated with order to checked out
    if (order?.cartId) {
      await this.prisma.cart.update({where: {id: order.cartId}, data: { checkedOut: true } });
    }
    return { success: true,
  ...
  async fetchPaymentById(paymentId: string, userId: string): Promise<{
    success: boolean; data: PaymentResponseDto; message: string; }>{
  ...
  async fetchPaymentByOrderId(orderId: string, userId: string): Promise<{
    success: boolean; data: PaymentResponseDto | null; message: string; }>{
    const payment = await this.prisma.payment.findFirst({where: { orderId, userId }});
    if (!payment) {
