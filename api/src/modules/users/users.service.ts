...
export class UsersService {
  ...
  // Update user profile
  async upateUserProfile(userId:string, updateUserDto: UpdateUserDto): Promise<UserResponseDto>{
    ...
    if (updateUserDto.email && updateUserDto.email !== existingUser.email) {
      const isEmailTaken = await this.prisma.user.findUnique({ where: { email: updateUserDto.email }});
      if (isEmailTaken) {
  ...
  async changeUserPassword(userId:string, changePasswordDto: ChangePasswordDto): Promise<{message:string}>{
    ...
    if (isSamePassword) {
      throw new NotFoundException(' The New password  must be different from the old password');
    }
