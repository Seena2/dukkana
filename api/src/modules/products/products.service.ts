...
export class ProductsService {
  ...
  async createNewProduct(createProductDto: CreateProductDto): Promise<ProductResponseDto> {
    const { sku, price } = createProductDto;
    const SkuAlradyExist = await this.prisma.product.findUnique({ where: { sku } });
    if (SkuAlradyExist) {
  ...
  async updateProduct( id: string, updateProductDto: UpdateProductDto): Promise<ProductResponseDto> {
    ...
    if (updateProductDto.sku && updateProductDto.sku !== existingProduct.sku) {
      const skuTaken = await this.prisma.product.findUnique({ where: { sku: updateProductDto.sku }})
      if (skuTaken) {
    ...
    const updatedProduct = await this.prisma.product.update({ where: { id },
      data: updateData,
      include: { category: true },
    });
  ...
  async deleteProduct(id: string): Promise<{ message: string }> {
    ...
    if (product.orderItems.length > 0) {
      throw new BadRequestException(' Cannot delete product that is part of existing order. consider marking it as inactive');
    }
  ...
  async fetchAllProduct(queryProductDto: QueryProductsDto): Promise<{
    ...
    const { category, isActive, search, page = 1, limit = 10 } = queryProductDto;
